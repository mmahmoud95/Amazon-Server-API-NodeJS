...
const getallSub_subcategory = async (req, res) => {
        try {
            var sub_subCategory = await sub_subCategorymodel.find().populate("SubCategoryid")
...
const getSub_subcategoryByid = async (req, res) => {
    ...
    try {
        const sub_subCategory = await sub_subCategorymodel.findOne({
            _id: id
...
const subOfSubCategoery = async (req,res)=>{
    var parentSubCategory = req.params.id
    try{
        const subOfsubCtegorie = await sub_subCategorymodel.find(parentSubCategory).populate("SubCategoryid")
...
const saveSub_subcategory = async (req, res) => {
    ...
    console.log(req.id)
    try {
        var new_sub_subCategory = await sub_subCategorymodel.create(sub_subCategory)
        console.log("Created At:", sub_subCategory.createdAt);
        console.log("Updated At:", sub_subCategory.updatedAt);
        res.status(201).json({  message: "sub_subCategory saved successfully",date: new_sub_subCategory })
    } catch (err) {
        res.status(400).json({
            message: err.message
    })
...
const patchSub_subcategory = async (req, res) => {
    ...
    var {id} = req.params;
    try {
        const queryRes3 = await sub_subCategorymodel.updateOne({ _id: id }, { name: name })
...
module.exports = { getallSub_subcategory, saveSub_subcategory, getSub_subcategoryByid, patchSub_subcategory, deletSub_subcategory,subOfSubCategoery}
